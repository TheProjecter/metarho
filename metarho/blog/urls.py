...
urlpatterns = patterns('metarho.blog.views',
    ...
    url(r'^(?P<slug>[\w\-]+)/$', 'post_topic', name='post-topic'),
    url(r'^(?P<slug>[\w\-]+)/(?P<year>\d{4})/$', 'post_topic_year', name='post-topic-year'),
    url(r'^(?P<slug>[\w\-]+)/(?P<year>\d{4})/(?P<month>\w{3})/$', 'post_topic_month', name='post-topic-month'),
    url(r'^(?P<slug>[\w\-]+)/(?P<year>\d{4})/(?P<month>\w{3})/(?P<day>\d{1,2})/$', 'post_topic_day', name='post-topic-day'),
